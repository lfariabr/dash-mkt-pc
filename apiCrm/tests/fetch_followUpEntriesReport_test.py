...
async def main():
    ...
    print(f"Fetching follow-up entries report from {start_date} to {end_date}...")
    
    # Call the function we want to test
    appointments = await fetch_and_process_followUpEntriesReport(start_date, end_date)
    
    # Print summary of results
    print(f"Successfully fetched {len(appointments)} follow-ups entries")
    
    # Print first appointment as sample (if available)
    if appointments and len(appointments) > 0:
        print("\nSample follow-up entries data:")
