...
async def fetch_followUpEntriesReportTest(session, start_date: str, end_date: str) -> List[Dict]:
    ...
    try:
        if 'data' in data and 'followUpEntriesReport' in data['data']:
            ...
            page_transformed = process_followUpEntries_data(appointments_data, start_date, end_date)
            all_followUpEntries.extend(page_transformed)
            
            last_page = meta.get('lastPage', 1)
            total_records = meta.get('total', 0)
            
            logger.info(f"Successfully fetched page 1/{last_page}, got {len(appointments_data)} appointments out of approximately {total_records}")
...
def process_followUpEntries_data(appointments_data, start_date, end_date):
